...
def build_amount_from_config(df, config):
    ...
    if mode == "amount":
        return df[config["amount_col"]].apply(clean_amount)

    elif mode == "debit":
        return df[config["debit_col"]].apply(clean_amount)

    elif mode == "credit":
        return df[config["credit_col"]].apply(clean_amount)

    elif mode == "combine":
        debit = df[config["debit_col"]].apply(clean_amount)
        credit = df[config["credit_col"]].apply(clean_amount)
        return debit + credit
